...
3. The lifecycle hardening separates byte acquisition from admission provenance, keeps authoritative SRA payloads after FASTQ derivation, and makes attestations policy-versioned. Native initialization is transactional; legacy adoption verifies the complete SRA set before publishing identity, ledger, or attestation, so a failed bootstrap can be retried without partial managed state.
4. Status is deliberately conservative: it validates the full historical attestation chain and evaluates the latest completed attestation against current control state and a quick payload sentinel. A fresh cryptographic claim requires `verify`; deep verification additionally requires `vdb-validate`.
